...
**Note**: As a mentioned, this loop executes for every element in an array. It can't use for objects, for example:
...
As you can see, it got all keys (index) from `names` array. It can very useful for getting property's values in objects, for example:
...
Don't do this; it crashed your editor your browser (or other). Let's try another example:
...
**JavaScript** have some **label** statements:
...
As you can see, the `for` loop skip in fifth time iterating and skip printing `5` in output.
