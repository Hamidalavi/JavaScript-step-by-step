...
We can fix the previous problem (last example) by `find()` method. It takes an function that it have three arguments (parameters), first for vlaue, second for index and third for entire array, for example:
