...
We can also access the CSS properties by `style`:
...
We also can use `ul.lastElementChild.before(newLi);` and `ul.lastElementChild.replaceWith(newLi);`. These are different ways of inserting this in different places.
