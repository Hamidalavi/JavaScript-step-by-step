...
The inner `result` is seprate that outer `result`. They are completely independent. You must first read **JavaScript-tips** file (repo) to understand that. Don't be lazy, first read that then read **JavaScript-step-by-step** file.
