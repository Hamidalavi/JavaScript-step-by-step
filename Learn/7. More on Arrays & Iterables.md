...
Instead to retrieve a value from the `Set`, we can use one of these specific `Set` methods which you see if we type a `.` dot here (after `Set` SetConstructor). We can use `.has()` method for check the values existance:
