...
What is `...` operator? We call it **rest operator**. With this `...` operator, they're (parameters) getting merge into an array inside of the function.

**Note**: Don't use built-in `arguments` keyword (pre-**ES6**). We don't recommend it to use (some developers do that).

We can't add parameters after the rest operator:
