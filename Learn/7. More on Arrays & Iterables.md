...
set get keys values size (key-value pair)

Now how about `Map`? The `Map` object holds **key-value** pairs and remembers the original insertion order of the keys. Any value (both objects and primitive values) may be used as either a key or a value.
