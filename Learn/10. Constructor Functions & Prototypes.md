...
For example: If `p.breathe()` not exists (not defined), way of searching will change to `p.__proto__.breathe()`; if this method still not exists (not defined in `Person`’s prototype, it searches for `p.__proto__.__proto__.breathe()`.
