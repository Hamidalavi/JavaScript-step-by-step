...
We only tell **TypeScript** that we select an element in the DOM by `id`و but this could be an element; we can have any `id` on any element, it could be a paragraph, it could be a button, it could be a list item, it doesn't have to be an input element. If we hover over `getElementById()`, we indeed see